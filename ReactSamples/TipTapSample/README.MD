...
TipTap is a modern, headless rich-text editor built on ProseMirror. It provides a lightweight, extensible API and a collection of ready-made extensions so you can build customizable editors (including React-based UIs) while keeping full control over behavior and presentation.

## Installation

Prerequisites

- React 16.8+ (hooks) and Node.js/npm or yarn.
...
- Add any extra extensions you need (for example `@tiptap/extension-underline`, `@tiptap/extension-link`, `@tiptap/extension-history`) the same way.
- No global CSS is required by core TipTap; extensions or your UI may add styles.
...
Create a new component and `src/components/ArticleEditor.tsx`
...
You can run the the the app by `npm run dev` command.
